...
function editUser() {
    ...
    let id = window.formEditUser.Id.value;

    let eRoles = window.formEditUser.eRoles.value;

    let newRole =[];
    for (const r in eRoles.roles){
        newRole.push(r);
    }
  ...
  fetch('http://localhost:8080/admin/update/'+ id,{
      ...
      body: JSON.stringify({
          ...
          firstName: window.formEditUser.FirstName.value,
          lastName: window.formEditUser.Lastname.value,
          age: window.formEditUser.Age.value,
      ...
      .then(response => {
          ...
          $('#' + id).replaceWith('<tr id=' + id + '>' +
              '<td>' + id + '</td>' +
              '<td>' + window.formEditUser.FirstName.value + '</td>' +
              '<td>' + window.formEditUser.Lastname.value + '</td>' +
              '<td>' + window.formEditUser.Age.value + '</td>' +
              '<td>' + window.formEditUser.Email.value + '</td>' +
              '<td>' + window.formEditUser.eRoles.value + '</td>' +
